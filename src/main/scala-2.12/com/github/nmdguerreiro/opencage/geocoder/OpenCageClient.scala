 ...
 *
 * The client auto-closes its resources if used within a try-with-resources block. Otherwise, you should call close when you're done with it to avoid resource leakage.
 *
 * @param authKey The auth key provided to you by OpenCage
 * @param scheme http/https
 * @param hostname Defaults to com.github.nmdguerreiro.opencage.geocoder.OpenCageClient#defaultHostname(), but can be overridden for testing purposes
 * @param backend The AHC backend is used by default. If you need to configure proxies, timeouts, connection pools, etc, you can do that yourself, by providing a custom backend. That can easily be done
 *                by calling com.softwaremill.sttp.asynchttpclient.future.AsyncHttpClientFutureBackend#apply(com.softwaremill.sttp.SttpBackendOptions, scala.concurrent.ExecutionContext) and/or by supplying it a custom configuration (com.softwaremill.sttp.asynchttpclient.future.AsyncHttpClientFutureBackend#usingConfig(org.asynchttpclient.AsyncHttpClientConfig, scala.concurrent.ExecutionContext)) before calling this constructor.
 */
class OpenCageClient(authKey: String,
        ...
        decode[OpenCageResponse](rawResp) match {
          ...
          case Right(errorResponse) => {
            ...
            httpResponse.code match {
              case 400 => Failure(new InvalidRequestError(s"Invalid request: ${errorResponse.status.message}"))
              case 402 => Failure(new QuotaExceededError(s"Quota exceeded: ${errorResponse.status.message}"))
              case 403 => Failure(new ForbiddenError(s"Forbidden: ${errorResponse.status.message}"))
              case 408 => Failure(new TimeoutError(s"Timeout: ${errorResponse.status.message}"))
              case 410 => Failure(new RequestTooLongError(s"Request too long: ${errorResponse.status.message}"))
              case 429 => Failure(new RateLimitExceededError(s"Request too long: ${errorResponse.status.message}", errorResponse.rate.map(_.reset)))
  ...
  private def buildUri(query: String, params: OpenCageClientParams): Uri = {
    ...
    if (!params.countryCodes.isEmpty) extraParams += ("countrycode" -> params.countryCodes.mkString(","))
